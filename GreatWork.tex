...
{\color{brown}
In this section the physiological aspects of O2 transport through vessels into
the tissue will be discussed.  
This part is mainly dedicated to the understanding of physiological processes
that are involved in the transport of oxygen through capillaries into the
tissue, and its consumption therein.
}\\
...
\\
\\Blood is basically composed of two phases: liquid and cells. The liquid phase is called plasma, and the cells that are our main concern are the red blood cells. This will later play an important role when considering the oxygen transport in blood. For computational models, blood is generally modeled as an incompressible Newtonian fluid \cite{rahman2012mathematical}. This basically means that the viscous stress is linearly proportional to the strain rate. This assumption generally holds, even though the particles in the blood, which are the cells, start playing a more important role for the smallest vessels.
This leads to the Navier-Stokes equations \ref{eq:NavierStokes1}, \ref{eq:NavierStokes2}, which basically describe the flow dynamics of an incompressible Newtonian fluid:
\begin{equation}
...
\\Finish this chapter using the Rahman paper...
...
\\Papers to cite: \cite{rogersupdate}
...
In order to simulate flows, Computational Fluid Dynamics (CFD) is a tool that can be used to produce results which can not be obtained through in vitro or in vivo experiments  \cite{rahman2012mathematical}. For this reason and because the computational capabilities available are growing everyday, modeling and simulating processes in the body is taking an increasingly important role in biological and medical sciences.
...
\\(Paper to cite here:https://arxiv.org/pdf/1803.04896.pdf)
...
The Krogh model describes the tissue as a cylinder around the vessels and assumes that the capillaries are straight and parallel. The blood flow in all the capillaries is assumed to be unidirectional, and the distribution of capillaries is assumed to be homogeneous \cite{kreuzer1982oxygen}. Obviously this model has a few unrealistic assumptions, but as it is a straightforward network, it is the most basic model to start with.
...
Figure \ref{fig:Contour_Krogh} is the result of a Green's Method blood flow simulation computed on a Krogh network. The Solute Concentration in the network and in the tissue is visualized in the form of a slice (See Output Section \ref{Outputs} for more information). The illustration basically shows a virtual cut into the tissue and gives local oxygen concentrations in mmHg.\\
\begin{figure}[h]
\centering
...
\end{figure}
One can very clearly see that the results produced are physically very realistic. On \ref{fig:Contour_Krogh} the blood flow goes from left to right. The $O_2$ concentration in mmHg accordingly goes down in flow direction. The concentrations as well as the diffusive distance in the tissue also decrease in flow direction, which can be explained by the fact that a lower concentration gradient goes with a lower Diffusive Flux. This is makes sense when looking at Fick's Law of Diffusion \ref{Fick}. As one can see from the Input files \ref{Inputs}, the Diffusivity constant $\alpha$ is constant for homogeneous tissue. Due to the fact that the diameter of the vessels is constant as well when considering the Krogh Model, the intravascular resistance to radial oxygen transport K for each diameter/vessel is also constant. Differences in diffusive fluxes and Oxygen penetration distances into the tissue therefore mainly depend on the $PO_2$ gradients themselves.
...
\end{figure}
\\Obviously looking at the Contour plot doesn't make much sense here, as the high number of vessels makes an analysis of the $PO_2$ impossible. For this reason, other files must be used but which ones ?

\newpage
\subsubsection*{The Mesentery Artery Model}
%Explain relevance of this model and the produced results (many plots here)

The Mesentery Artery network provided by Secomb was used for further simulations and to get more computational results.\\
\\This network is obviously more complicated than the previously presented Krogh model but luckily gives a way better insight to the results than the Cardiac Network simulation, due to the much lower vessel density. The result is visualized on Figure \ref{fig:Contour_Mesent1}. Here on can see the result of a Green's Method blood flow simulation computed on a Mesentery Artery network. As for the previous models, the Oxygen Concentration in the network and in the tissue is visualized in the form of a slice (See Output Section \ref{Outputs} for more information). Again the illustration shows a virtual cut into the tissue and gives local oxygen concentrations in mmHg.\\\\
\begin{figure}[h]
\centering
\includegraphics[width=85mm]{Contour_Mesent1}
\caption{\footnotesize Green's Contour Output for a Mesentery Artery Network (Solute 1)}
\label{fig:Contour_Mesent1}
\end{figure}\\
%
%\\Figure \ref{fig:Contour_Mesent2}  is the result of a Green's Method blood flow simulation computed on a Mesentery Artery network. The Solute Concentration of Solute 2 in the network is visualized.\\
%\begin{figure}[h]
%\centering
%\includegraphics[width=85mm]{Contour_Mesent2}
%\caption{\footnotesize Green's Contour Output for a Mesentery Artery Network (Solute 2)}
%\label{fig:Contour_Mesent2}
%\end{figure}
\\Due to the low density of capillaries for this network, this figure can give a good insight to the values computed by the Green's Model. One can clearly see that partial pressure of $O_2$ goes down in radial direction with growing distance from the vessels. At hotspots where the capillary density is higher due to branching and ramification, the diffusion comes from two vessels and the superposition of two close source elements obviously leads to peaks in $PO_2$ values.

\newpage
\subsubsection*{A First Tumor Model}
%Explain relevance of this model and the produced results (many plots here)
...
\begin{figure}[h]
\centering
\includegraphics[width=75mm]{Contour_TumorDuke}
...
\caption{Pressure Field in a Glomerulus}
