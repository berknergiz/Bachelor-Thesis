...
\\The previously described tasks can be all be done by Interface Group members, which makes the working environment very special in terms of independent research capabilities in this partially uncharted and unexplored field.
...
This thesis aimed to understand and specify how computationally blood flow and oxygen transport can be simulated using different methods. For this, two approaches have been presented and the results or outlooks have been discussed.\\
\\While oxygen transport is a crucial topic in the fields of medicine and physiology, many processes in microvascular networks are not understood yet. Especially the measurements of microvascular and tissue oxygen concentrations is an uncharted research field. For this reason, a lot of research is being done in oxygen distribution computations. In view of the challenges that are faced when simulating oxygen fields in tissue surrounding vessel networks, I would like to split this last section into three parts: a technical discussion, a physical discussion, and finally a physiologic discussion.

\subsection{Technical Dicussion}
...
\\When building a computational model like the models used in this thesis, one has to start by studying the physical equations describing the given problem. In our case, the physical equations describing the blood flow, and accordingly the oxygen transport from the vessel to the tissue have been used in general. Solving these equations will then yield the results that the user wanted to compute. This basic procedure is an engineering procedure, and is the same for a flow and heat exchange computation in a pipe or a blood flow and oxygen diffusion computation in a tissue. Due to this fact, engineers are strongly engaged in this research field, as they have a good knowledge about the technical tools and have a good understanding for physical quantities. Also, the used networks and fluiddynamic challenges can be very interesting in this physiologic field, as for example the complex vascular network found in the glomerulus, where pressure differences and gradients were very interesting to observe and required deeper understanding of the Navier-Stokes equations, as the pressure drop was nothing compared to the one in a straight pipe.\\
...
\subsection{Physiological Discussion}
...
\\As the methods and the produced outputs were presented in the previous sections, one could clearly see that the computed results were physiologically very interesting. With computed oxygen concentrations ranging from $0$ mmHg to around $87$ mmHg in the tissue, the values seemed realistic and the distributions inside the tissue seemed to be qualitatively correct. The pressure gradients computed by the $DuMu^x$ model were also analyzed and the general pressure field could physiologically be explained. Again, we do not really have experimental values to compare the computed values with, so most of the results can only be analyzed qualitatively.
\\Especially the oxygen fields computed by the Green's method in some complex networks produced very interesting and physiologically useful results. Even though we don't have experimental data to compare the computational values with, it is possible to see that the results were all in the physiologic range and the general oxygen distribution showed that the results were according to the physics describing our problem. Extrapolating from the examples presented for oxygen distribution computations, I would like to underline the fact that computational models are very useful in this specific field and will very likely take an even more important share in the future.
\\
