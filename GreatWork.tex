...
\end{figure}
One can very clearly see that the results produced are absolutely realistic. On \ref{fig:Contour_Krogh} the blood flow goes from left to right. The $O_2$ concentration in mmHg accordingly goes down in flow direction. The concentrations as well as the diffusive distance in the tissue also decrease in flow direction, which can be explained by the fact that a lower concentration gradient goes with a lower Diffusive Flux \ref{Fick}.
...
\end{figure}
\\Obviously looking at the Contour plot doesn't make much sense here, as the high number of vessels makes an analysis of the $PO_2$ impossible. For this reason, other files must be used what which ones ?

\newpage
\subsubsection*{The Mesentery Artery Model}
%Explain relevance of this model and the produced results (many plots here)

The Mesentery Artery network provided by Secomb was used for further simulations and to validate the produced data.\\
\\This network is obviously more complicated then the previously presented Krogh model. The result is visualized on Figure \ref{fig:Contour_Mesent1}. Here on can see the result of a Green's Method blood flow simulation computed on a Mesentery Artery network. As for the previous models, the Oxygen Concentration in the network and in the tissue is visualized in the form of a slice (See Output Section \ref{Outputs} for more information).\\
\begin{figure}[h]
\centering
...
%\end{figure}
\\Due to the low density of capillaries for this network, this figure can give a good insight to the values computed by the Green's Model. As for the previous models, the illustration shows a virtual cut into the tissue and gives local oxygen concentrations in mmHg.
