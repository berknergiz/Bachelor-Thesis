...
\end{figure}
One can very clearly see that the results produced are absolutely realistic. On \ref{fig:Contour_Krogh} the blood flow goes from left to right. The $O_2$ concentration (in what unit?) accordingly goes down in flow direction. The concentrations as well as the diffusive distance in the tissue also decrease in flow direction, which can be explained by the fact that a lower concentration gradient goes with a lower Diffusive Flux \ref{Fick}.
...
\newpage
\subsubsection*{The Mesentery Arterty Model}
%Explain relevance of this model and the produced results (many plots here)

The Mesentery Artery network provided by Secomb was used for further simulations and to validate the produced data.\\
\\Figure \ref{fig:Contour_Mesent1}  is the result of a Green's Method blood flow simulation computed on a Mesentery Artery network. The Solute Concentration of Solute 1 in the network is visualized.\\
\begin{figure}[h]
\centering
