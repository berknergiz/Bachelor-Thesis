...
{\color{brown} This is basically the most important part of your whole thesis.
Why not make it a separate section, instead of subsubsection? 
Also, the title of the section may be changed to: Simulation results and analysis.
}\\
...
\label{fig:tracer_2}
\end{figure}
Figure \ref{fig:tracer_2}  is the result of a $DuMu^x$ tracer simulation computed on a rectangular geometry. The solute field in the network for the last time step is visualized.\\
\begin{figure}[h]
\centering
...
\end{figure}
Figure \ref{fig:tracer_velocity}  is the result of a $DuMu^x$ tracer simulation computed on a rectangular geometry. The velocity field of the solute, which is constant, in the network for all time steps is visualized.\\
