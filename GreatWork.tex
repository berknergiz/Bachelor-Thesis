...
\\Even though it is possible to observe the same pressure drop when analyzing the pressure field in Glomerulus 2, the rather constant fall in pressure is physically hard to explain. For this glomerulus, I would like to point out the fact that the network data of the afferent and efferent arteriols was corrupted, and thus these parts look rather short, but can still be seen on the figure. Also the fact that some vessels inside the glomerulus and the glomerular network have lower pressures than the pressure computed for the efferent arteriol is an interesting observation. This can also be seen for Glomerulus 3 (figure \ref{fig:glom3_pressure}) and Glomerulus 4 (figure \ref{fig:glom4_pressure}).
\\Generally, it is possible to observe that the pressure constantly drops over the afferent and the efferent arteriols. For this to happen, we physically need a constantly growing vessel diameter. As previously mentioned, the efferent arteriol has a bigger diameter than the efferent arteriol in average, which causes the overall pressure difference. It is hard to find an explanation for the pressure drop inside segments of both afferent or efferent arteriols, which must be due to diameter changes.
\\
